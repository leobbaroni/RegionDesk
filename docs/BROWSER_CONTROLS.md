...
- **Star** adds/removes the current page from this profile's bookmarks. Middle-click a saved bookmark to open a new tab; the small X removes it.
...
HTTP 429 displays a cooldown. A still-valid session survives transient rate limits, but the verification lease does not extend; an expired lease locks browsing. Other route/authentication failures lock the browser. No direct network fallback is enabled.
