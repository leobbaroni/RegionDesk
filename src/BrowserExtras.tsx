...
export default function BrowserExtras({ data, busy, run }: { data: AppState; busy: boolean; run: (fn: () => Promise<unknown>) => void }) {
  ...
  return <>
    <div className="bookmark-bar" aria-label="Bookmarks and page tools">
      ...
      <div className="bookmark-items"><button disabled={!ready || busy} onClick={() => run(() => api!.navigate('https://www.tiktok.com/'))}>TikTok</button><button disabled={!ready || busy} onClick={() => run(() => api!.navigate('https://www.tiktok.com/tiktokstudio/upload'))}>Upload</button>{bookmarks.length ? bookmarks.map(entry => <span className="bookmark-item" key={entry.url}><button disabled={!ready || busy} title={entry.url} onClick={() => run(() => api!.navigate(entry.url))} onAuxClick={event => { if (event.button === 1 && ready) run(() => api!.newTab(entry.url)); }}>{entry.title}</button><button aria-label={`Remove bookmark ${entry.title}`} disabled={busy} onClick={() => run(() => api!.toggleBookmark(entry.url, entry.title))}><X size={11} /></button></span>) : <span className="bookmark-hint">Save your pages with the star.</span>}</div>
